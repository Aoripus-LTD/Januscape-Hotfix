...
适用于 Linux 4.0–6.x，所有发行版。通过 ftrace 在线替换 `kvm_mmu_get_child_sp()`，
无需重启、无需迁移虚拟机。

## 前置条件

- **Linux ≥ 4.0**（`ftrace` 需要 `FTRACE_OPS_FL_IPMODIFY`）
...
detect offsets → make → insmod        Hook kvm_mmu_get_child_sp()
BTF → DWARF → DB → defaults           ftrace_direct (5.11+) → IPMODIFY (4.0+)
                                      kallsyms_lookup_name + kprobe fallback
```
...
| 内核版本 | 方法 |
|---------|------|
| ≥ 5.14 | `register_ftrace_direct(ops, addr)` |
| 5.11–5.13 | `register_ftrace_direct(ip, addr)` |
| 4.0–5.10 | ftrace IPMODIFY + SAVE_REGS |
| < 4.0 | 不支持 |
...
| `register_ftrace_direct: -22` | 函数未找到或已修补 | `grep kvm_mmu_get_child_sp /proc/kallsyms` |
